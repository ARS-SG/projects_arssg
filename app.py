...
client = OpenAI(api_key="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")
    ...
    if "capabilities" in user_input.lower():
        ...
        if len(user_responses) < len(initial_questions):
            ...
            if ocean_question_counter < len(ocean):
                ...
                aptitude_question_counter = 0
                return aptitude_questions[0]
        elif aptitude_question_counter < len(aptitude_questions):
            aptitude_responses.append(int(user_input))
            aptitude_question_counter += 1
            if aptitude_question_counter < len(aptitude_questions):
                return aptitude_questions[aptitude_question_counter]
            else:
...
def handle_results():
    ...
    return render_template('result.html', model_output=model_output, ocean_averages=ocean_averages, openai_response=openai_response)

# Function to get OpenAI response
def get_openai_response():
    prompt = "User: Do you have any other questions?\nAI: "
    chat_completion = client.chat.completions.create(
        messages=[
            {"role": "user", "content": prompt},
            {"role": "system",
             "content": "You are expert in Indian education system and student career guidance, answer prompts like it."}
        ],
        model="gpt-3.5-turbo"
    )
